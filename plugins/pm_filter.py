...
async def next_page(bot, query):
    ...
    btn.append([
        InlineKeyboardButton(text="⏪ Back", callback_data=f"next_{req}_{key}_{orginal_offset}"),])
    await query.message.edit_text(cap + links + js_ads, disable_web_page_preview=True, parse_mode=enums.ParseMode.HTML, reply_markup=InlineKeyboardMarkup(btn))
    return
...
async def cb_handler(client: Client, query: CallbackQuery):
    if query.data == "close_data":
        ...
        buttons = [
    [
        InlineKeyboardButton('📢 Quick Help', callback_data='help'),
        InlineKeyboardButton('🕵️‍♂️ Admin Support', callback_data='source')
    ],
    [
        InlineKeyboardButton('💡 Movie Ideas', callback_data="free"),
        InlineKeyboardButton('🎟️ Upgrade Plan', callback_data="jisshupremium")
    ]
]
        reply_markup = InlineKeyboardMarkup(buttons)
...
#premium plan
        ...
        buttons = [[
            InlineKeyboardButton('🕵️‍♂️ Contact Admin ', url='https://telegram.me/iam_Sandman'),
  #          InlineKeyboardButton('Admin Command', callback_data='admin_command')
        ...
        await query.message.edit_text(
            ...
            btn = [[
                    InlineKeyboardButton("📲 Contact Admin ",  url=f'https://telegram.me/{OWNER_USERNAME}')
               ],[
                    InlineKeyboardButton("⏪ Back", callback_data="start")
                  ]]
            reply_markup = InlineKeyboardMarkup(btn)
       ...
       await client.edit_message_media(
        ...
        buttons = [[
            InlineKeyboardButton('⏪ Back', callback_data='special')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)  
        await query.message.edit_text(
...
async def auto_filter(client, msg, spoll=False , pm_mode = False):
    if not spoll:
        ...
        chat_id = message.chat.id
        settings = await get_settings(chat_id)
        searching_msg = await msg.reply_text(f' Loading Files 🔎 {search}')
        files, offset, total_results = await get_search_results(search)
        await searching_msg.delete()
        if not files:
            if settings["spell_check"]:
                ai_sts = await msg.reply_text(f'You Misspelled, I Am Trying To Find')
    ...
    if imdb:
        ...
        cap = f"<b>🪐 Result For : {message.from_user.mention}</b></b>"
