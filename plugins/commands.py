...
from pyrogram.types import InlineKeyboardButton, InlineKeyboardMarkup , ForceReply, ReplyKeyboardMarkup, CallbackQuery, WebAppInfo
...
async def start(client:Client, message):
    ...
    [
        InlineKeyboardButton('💡 Movie Ideas',web_app=WebAppInfo(url="https://toji-v1.vercel.app")),
        InlineKeyboardButton('🎟️ Upgrade Plan', callback_data="premium")
    ]
]
    ...
    if settings.get('fsub_id', AUTH_CHANNEL) == AUTH_REQ_CHANNEL:
        if AUTH_REQ_CHANNEL and not await is_req_subscribed(client, message):
            ...
            if message.command[1] != "subscribe":
                btn.append([InlineKeyboardButton("✅ I Am Joined ✅", url=f"https://t.me/{temp.U_NAME}?start={message.command[1]}")])
    ...
    if data and data.startswith("allfiles"):
        ...
        for file in files:
            ...
            btn = [[
        InlineKeyboardButton("💻 Watch Online / Download 📩v", callback_data=f'stream#{file_id}')
    ]]
